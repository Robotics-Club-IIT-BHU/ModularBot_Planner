...
class OptionCriticConv(nn.Module):
    def __init__(self,
        ...
        self.terminations = nn.Linear(512, num_actions)
        self.option_W = nn.Parameter(torch.zeroes(num_options, 512, num_actions))
...
class OptionCriticFeatures(nn.Module):
    ...
    def predict_option_termination(self, state, current_option):
        termination = self.termination(state)[:, current_option].sigmoid()
        option_termination = Bernoulli(termination).sample()
        Q = self.get_Q(state):
        next_option = Q.argmax(dim=-1)
        return bool(option_termination.item()), next_option.item()
    ...
    def epsilon(self):
        if not self.testing:
            eps = self.eps_min + (self.eps_start - self.eps_min)  * eps(-self.num_steps/self.eps_decay)
            self.num_steps += 1
...
def actor_loss(obs, option, logp, entropy, reward, done, next_obs, model, model_prime, args):
    ...
    next_staet_prime = model_prime.get_state(to_tensor(next_obs))

    option_term_prob = model.get_terminations(state)[:, option]
    next_options_term_prob = model.get_terminations(next_state)[:, option].detach()
    ...
    gt = reward + (1 - done) * args.gamma * \
        ((1 - next_options_term_prob) * next_Q_prime[option] + next_options_term_prob * next_Q_prime.max(dim=-1)[0])
